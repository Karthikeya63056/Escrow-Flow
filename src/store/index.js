  ...
  escrows: [
    {
      ...
      description: "Complete redesign of the storefront with modern UI, payment integration, and admin dashboard. Must support mobile-first responsive design.",
    ...
    },
    {
      ...
      history: [
        { action: "created", by: "Alex Morgan", at: new Date(Date.now() - 1 * 86400000).toISOString() },
      ],
    },
    {
      id: "esc-005",
      ...
      buyer: { id: "user-1", name: "Alex Morgan" },
      seller: { id: "user-7", name: "DevWriter Pro" },
      arbitrator: { id: "user-3", name: "Mike Torres" },
      amount: 4500,
      funded: 4500,
      released: 1500,
  ...
  approveMilestone: (escrowId, milestoneId) =>
    set((s) => ({
      escrows: s.escrows.map((e) => {
        ...
        const ms = e.milestones.find((m) => m.id === milestoneId);
        if (!ms) return e;
        return {
          ...e,
          released: e.released + ms.amount,
          milestones: e.milestones.map((m) =>
            m.id === milestoneId ? { ...m, status: "approved", completedAt: new Date().toISOString() } : m
          ),
          history: [
            ...e.history,
            { action: "milestone_approved", by: "Alex Morgan", at: new Date().toISOString(), detail: `${ms.title} — $${ms.amount.toLocaleString()} released` },
          ],
        };
      }),
    })),
      ...
      escrows: [
        {
          ...
          history: [{ action: "created", by: "Alex Morgan", at: new Date().toISOString() }],
        },
        ...s.escrows,
      ],
    })),
